 ...
 *
 */
public class BatterySensorTest {
	...
	private static final String NAME = "Battery Sensor";
	...
	private PolysunSettings createPolysunSettings(String host, int port,
			int timestampSetting, int waitForRsp, boolean measuredSoc, boolean measuredBatteryTransfer, boolean sendTimestamp) {
		...
		sensors.add(new Sensor(SENSOR1, "", true, false, measuredSoc)); // Sensor 1
		sensors.add(new Sensor(SENSOR2, "W", true, false, measuredBatteryTransfer)); // Sensor 2
	...
	private PolysunSettings createPolysunSettingsDefaultConfiguration() {
		return createPolysunSettings(DEF_TCP_ADDRESS, DEF_PORT_NUMBER, 0, 0, true, true, true);
	}
	...
	private PolysunSettings createPolysunSettingsFromConfiguration() throws PluginControllerException {
		...
		for (Sensor sensor : configuration.getSensors()) {
			sensors.add(new Sensor(sensor.getName(), sensor.getUnit(), sensor.isAnalog(), sensor.isRequired(),
					SENSOR1.equals(sensor.getName()) || SENSOR2.equals(sensor.getName())));
		}
	...
	public void setUp() throws Exception {
		controller = new BatterySensorController();
		echo = new EchoActor(2);
	}
	...
	public void testGetConfiguration() throws PluginControllerException {
		...
		assertEquals("Wrong number of configured sensors", 2, configuration.getSensors().size());
	...
	public void testGetControlSignalsToHide() {
		...
		List<String> controlSignalsToHide = controller.getControlSignalsToHide(
				createPolysunSettings(DEF_TCP_ADDRESS, DEF_PORT_NUMBER, state, 0, true, true, true), null);
	...
	public void testGetPropertiesToHide() {
		...
		List<String> propertiesToHide = controller
				.getPropertiesToHide(createPolysunSettings(DEF_TCP_ADDRESS, DEF_PORT_NUMBER, state, 0, true, true, true), null);
	...
	public void testGetSensorsToHide() {
		...
		List<String> sensorsToHide = controller
				.getSensorsToHide(createPolysunSettings(DEF_TCP_ADDRESS, DEF_PORT_NUMBER, state, 0, true, true, true), null);
	...
	public void testPolysunSettingsGetter() throws PluginControllerException {
		...
		assertEquals("Wrong sensor index returned", 1,
				((AbstractPluginController) controller).getSensorIndex(SENSOR2));
	...
	public void testControl() throws Exception {
		float[] sensors = new float[] {5, 6};
		float[] controlSignals = new float[0];
		float[] logValues = new float[3];
		int simulationTime = 0;
		controller.build(createPolysunSettings(DEF_TCP_ADDRESS, DEF_PORT_NUMBER, 0, 1, true, true, true), null);
		echo.start();
